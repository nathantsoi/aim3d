...
def test_daemon_gcode_simulation_compact_spaceless():
    ...
    try:
        ...
        # 2. The simulate endpoint no longer returns a headless cut mesh:
        #    material-removal cutting is frontend-only (WebGPU voxelizer).
        ...
        with urlrequest.urlopen(req, timeout=5.0) as resp:
            ...
            assert body["simulation"]["status"] == "unsupported"
    finally:
        server.shutdown()
        server.server_close()
