...
Welcome to the aim3d documentation suite. aim3d is an integrated 3D CAD, CAM, and CNC Control platform designed for modern, high-performance manufacturing environments, targeting an STM32 microcontroller for real-time step/dir execution.
...
4. **STM32 Firmware (C)**: Bare-metal execution on the STM32 microcontroller. Handles real-time Step/Dir generation, E-stop, and hardware interfacing. Firmware source lives in the sibling `aic3d` project; aim3d's controller daemon talks to it over serial.
...
*   **[Controller & Firmware](controller.md)**: Details on the Python REST daemon, STM32 integration, and deploying to hardware.
