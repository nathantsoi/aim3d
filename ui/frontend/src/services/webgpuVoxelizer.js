export const createWebGpuVoxelizer = async (device, userGridSize = [512, 512, 128], stockSize = [100, 100, 25], stockLocation = [0, 0, 0], uiScale = 1.0) => {
  ...
  const triMatch = text.match(/triTable\s*=\s*new\s*Int32Array\(\s*\[([\s\S]*?)\]\s*\)/);
  const edgeTableData = new Int32Array(edgeMatch[1].split(',').map(x => parseInt(x.trim(), 16) || 0));
  const triTableData = new Int32Array(triMatch[1].split(',').map(x => parseInt(x.trim(), 10) || 0));
  ...
  const maxTriangles = Math.floor(numVoxels * 0.20); // Estimate up to 20% of voxels contain triangles
  ...
  const updateParams = (numCuts) => {
    ...
    udata[0] = gridSize[0]; udata[1] = gridSize[1]; udata[2] = gridSize[2];
    udata[3] = numCuts;
  ...
  const initPipeline = createCompute(initCode, [device.createBindGroupLayout({
    entries: [
  ...
  const cutPipeline = createCompute(cutCode, [device.createBindGroupLayout({
    entries: [
      { binding: 0, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'storage' } },
  ...
  const mcPipeline = createCompute(mcCode, [device.createBindGroupLayout({
    entries: [
  ...
  pass.dispatchWorkgroups(Math.ceil(gridSize[0] / 8), Math.ceil(gridSize[1] / 8), Math.ceil(gridSize[2] / 4));
  pass.end();
  ...
  return {
    ...
    get volumeRemoved() { return totalVolumeRemoved; },

    applyCuts(cuts) {
      if (!cuts || cuts.length === 0) return;
      if (cuts.length > 0) {
        console.log(`[Voxelizer] applying ${cuts.length} cuts. First cut: ${JSON.stringify(cuts[0])}`);
      }
    ...
    async extractMesh() {
      ...
      totalVolumeRemoved = voxelsRemoved * (voxelSize[0] * voxelSize[1] * voxelSize[2]);
      if (voxelsRemoved > 0 && totalVolumeRemoved !== lastLoggedVolume) {
        lastLoggedVolume = totalVolumeRemoved;
        console.log(`[Voxelizer] Volume removed: ${totalVolumeRemoved.toFixed(2)} mm³ (${voxelsRemoved} voxels)`);
      }
    },
